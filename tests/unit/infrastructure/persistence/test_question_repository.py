...
class TestGetMostAskedQuestionFromPb:
    ...
    def test_returns_default_four_questions(self, repo):
        records = [make_pb_record(str(i), f"Q{i}") for i in range(4)]
        repo.client.collection.return_value.get_list.return_value = make_pb_list(*records)
    ...
    def test_returns_custom_number_of_questions(self, repo):
    ...
    def test_passes_correct_sort_param(self, repo):
        repo.client.collection.return_value.get_list.return_value = make_pb_list()

        repo.get_most_asked_question_from_pb(num_of_questions=3)

        repo.client.collection.return_value.get_list.assert_called_once_with(
            page=1,
            per_page=3,
            query_params={"sort": "-times_asked"},
        )

    def test_maps_records_to_question_objects(self, repo):
        record = make_pb_record("123124", "This is a question")
        repo.client.collection.return_value.get_list.return_value = make_pb_list(record)
    ...
    def test_returns_empty_list_when_no_records(self, repo):
        repo.client.collection.return_value.get_list.return_value = make_pb_list()

        assert repo.get_most_asked_question_from_pb() == []

    def test_raises_question_fetch_error_on_exception(self, repo):
        repo.client.collection.return_value.get_list.side_effect = RuntimeError("DB down")
    ...
    def test_raises_with_custom_count_in_message(self, repo):
        repo.client.collection.return_value.get_list.side_effect = RuntimeError("DB down")
...
class TestGetRandomQuestions:
    ...
    def test_exclude_ids_appear_in_filter(self, repo):
        ...
        filter_str = (
            repo.client.collection.return_value.get_list
            .call_args.kwargs["query_params"]["filter"]
        )
        assert 'id != "id1"' in filter_str
        assert 'id != "id2"' in filter_str

    def test_empty_exclude_produces_empty_filter(self, repo):
        repo.client.collection.return_value.get_list.return_value = make_pb_list()
    ...
    def test_sort_param_is_random(self, repo):
        repo.client.collection.return_value.get_list.return_value = make_pb_list()
    ...
    def test_raises_question_fetch_error_on_exception(self, repo):
        repo.client.collection.return_value.get_list.side_effect = RuntimeError("timeout")
...
class TestExistsByText:
    ...
    def test_returns_true_when_record_found(self, repo):
        repo.client.collection.return_value.get_first_list_item.return_value = MagicMock()

        assert repo.exists_by_text("This is a question") is True

    def test_returns_false_on_404_error(self, repo):
        repo.client.collection.return_value.get_first_list_item.side_effect = Exception("404 not found")

        assert repo.exists_by_text("Missing question") is False

    def test_returns_false_on_no_records_error(self, repo):
        repo.client.collection.return_value.get_first_list_item.side_effect = Exception("no records found")

        assert repo.exists_by_text("Missing question") is False

    def test_returns_false_on_not_found_error(self, repo):
        repo.client.collection.return_value.get_first_list_item.side_effect = Exception("record not found")

        assert repo.exists_by_text("Missing question") is False

    def test_raises_on_unexpected_exception(self, repo):
        repo.client.collection.return_value.get_first_list_item.side_effect = RuntimeError("connection refused")

        with pytest.raises(QuestionFetchError, match="Failed to check existence"):
            repo.exists_by_text("Some question?")

    def test_query_uses_correct_text(self, repo):
        repo.client.collection.return_value.get_first_list_item.return_value = MagicMock()

        repo.exists_by_text("Is TDD worth it?")

        repo.client.collection.return_value.get_first_list_item.assert_called_once_with(
...
class TestCreateQuestion:
    ...
    def test_calls_create_with_correct_payload(self, repo):
        repo.create_question("What is a question?")

        repo.client.collection.return_value.create.assert_called_once_with(
            {"question": "What is a question?", "times_asked": 0}
        )

    def test_times_asked_initialised_to_zero(self, repo):
        repo.create_question("Any question?")

        payload = repo.client.collection.return_value.create.call_args[0][0]
        assert payload["times_asked"] == 0

    def test_returns_none_on_success(self, repo):
        assert repo.create_question("New question") is None

    def test_raises_question_create_error_on_exception(self, repo):
        repo.client.collection.return_value.create.side_effect = RuntimeError("write failed")
...
class TestIncrementTimesAsked:
    ...
    def test_calls_update_with_correct_arguments(self, repo):
        repo.increment_times_asked("q123")

        repo.client.collection.return_value.update.assert_called_once_with(
            "q123", {"times_asked+": 1}
        )

    def test_increment_value_is_one(self, repo):
        repo.increment_times_asked("q42")

        payload = repo.client.collection.return_value.update.call_args[0][1]
        assert payload["times_asked+"] == 1

    def test_returns_none_on_success(self, repo):
        assert repo.increment_times_asked("q123") is None

    def test_raises_question_update_error_on_exception(self, repo):
        repo.client.collection.return_value.update.side_effect = RuntimeError("update failed")
